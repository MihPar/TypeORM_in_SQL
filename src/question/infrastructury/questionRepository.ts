...
export class QuestionRepository {
  ...
  async createQuestion(question: Question): Promise<Question | null> {
    try {
      const createQuestion = await this.dataSource
  ...
  async getQuestion(id: string): Promise<Question | null> {
    const findQuestion = await this.dataSource
      .getRepository(Question)
      .findOneBy({ id });
  ...
  async udpatedQuestionById(
    DTO: AnswerAndBodyClass,
    id: string,
  ): Promise<boolean | null> {
    const updateQuestionById = await this.dataSource
      .getRepository(Question)
      .createQueryBuilder()
      .update()
      .set({
  ...
  async updatePublished(id: string, published: boolean): Promise<boolean> {
    const updatePublished = await this.dataSource
      .getRepository(Question)
      .createQueryBuilder()
      .update()
