...
export class PairQuizGame {
  ...
  @OneToMany(() => QuestionGame, (questionGame) => questionGame.pairQuizGame, {
    nullable: true,
  })
  ...
  static quizGameViewModelForFirstPlayer(
    quizGame: PairQuizGame,
    login: string,
    id: string,
  ): GameTypeModel {
    return {
  ...
  static getViewModel(getGameById: PairQuizGame): GameTypeModel {
    return {
      id: getGameById.id,
      firstPlayerProgress: {
        answers: sortAddedAt<AnswersPlayer>(
          getGameById.firstPlayerProgress.answers,
        ).map((item) => AnswersPlayer.getViewModelForGame(item)),
        player: {
      ...
      secondPlayerProgress: getGameById.secondPlayerProgress
        ? {
            answers: sortAddedAt<AnswersPlayer>(
              getGameById.secondPlayerProgress.answers,
            ).map((item) => AnswersPlayer.getViewModelForGame(item)),
            player: {
        ...
        : null,
      questions: getGameById.questionGames.length
        ? getGameById.questionGames.map((item) => {
            return {
              id: item.question.id,
  ...
  static getViewModelPaging(
    getGameById: PairQuizGame,
    game: PairQuizGame,
  ): GameTypeModel {
    return {
      id: getGameById.id,
      ...
      secondPlayerProgress: getGameById.secondPlayerProgress
        ...
        : null,
      questions: game.questionGames.length
        ? game.questionGames.map((item) => {
            return {
              id: item.question.id,
              body: item.question.body,
            };
  ...
  static getViewModels(
    game: PairQuizGame,
    getGameFirstPlayer: PairQuizGameProgressPlayer,
    getGameSecondPlayer: PairQuizGameProgressPlayer,
  ): GameTypeModel {
    return {
      id: game.id,
      firstPlayerProgress: getGameFirstPlayer
        ? {
            answers: getGameFirstPlayer.answers.map((item) =>
              AnswersPlayer.getViewModelForGame(item),
            ),
            player: {
      ...
      secondPlayerProgress: getGameSecondPlayer
        ? {
            answers: getGameSecondPlayer.answers.map((item) =>
              AnswersPlayer.getViewModelForGame(item),
            ),
            player: {
