...
export class CreateOrConnectGameUseCase
  implements ICommandHandler<CreateOrConnectGameCommand>
{
  constructor(
  ...
  async execute(command: CreateOrConnectGameCommand): Promise<GameTypeModel> {
    ...
    const foundQuizGame = await this.pairQuizGameRepository.foundGame(
      GameStatusEnum.PendingSecondPlayer,
    );
    const getLoginOfUser = await this.usersQueryRepository.findUserById(
      command.userId,
    );
    ...
    if (!foundQuizGame) {
      ...
      const createProgressFirstPlayer =
        await this.pairQuizGameProgressRepository.createProgressFirstPlayer(
          progressFirstPlayer,
        );
      ...
      const createNewQuizGame = await this.pairQuizGameRepository.createNewGame(
        newQuizGame,
      );

      createProgressFirstPlayer.gameId = createNewQuizGame.id;
      await this.pairQuizGameProgressRepository.updateProgressFirstPlayer(
        createProgressFirstPlayer.id,
        createProgressFirstPlayer.gameId,
      );

      return PairQuizGame.quizGameViewModelForFirstPlayer(
        createNewQuizGame,
        firstLogin,
        command.userId,
      );
      ...
      await this.pairQuizGameProgressRepository.createProgressForSecondPlayer(
        progressSecondPlayer,
      );
      foundQuizGame.secondPlayerProgress = progressSecondPlayer;
      foundQuizGame.secondPlayerProgressId = progressSecondPlayer.id;

      const getFiveQuestionsQuizGame =
        await this.pairQuizGameRepository.getFiveQuestions(true);

      const questionGames: QuestionGame[] = getFiveQuestionsQuizGame.map(
        (item, index) => {
      ...
      const saveQuestions = await this.pairQuizGameRepository.createQuestions(
        questionGames,
      );
