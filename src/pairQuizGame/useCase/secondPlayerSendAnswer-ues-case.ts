...
export class SecondPlayerSendAnswerUseCase
  implements ICommandHandler<SecondPlayerSendAnswerCommand>
{
  constructor(
  ...
  async execute(command: SecondPlayerSendAnswerCommand): Promise<any> {
    if (command.activeUserGame.secondPlayerProgress.answers.length > 4) {
      ...
      const currentQuestionIndex: number =
        command.activeUserGame.secondPlayerProgress.answers.length;
      ...
      await this.pairQuizGameRepository.getProgressById(answer.progress.id);
      ...
      ); //.secondPlayerProgress = progressAfterUpdate

      const changeStatusToFinishedCommand = new ChangeStatusToFinishedCommand(
        command.game,
        command.game.questionGames.map((item) => {
