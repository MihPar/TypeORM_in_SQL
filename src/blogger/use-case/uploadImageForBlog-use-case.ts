...
export class UploadImageForBlogUseCase
  implements ICommandHandler<UploadImageForBlogCommand>
{
  constructor(
  ...
  async execute(command: UploadImageForBlogCommand): Promise<any> {
    ...
    if (command.mimetype !== ('image/jpeg' || 'image/jpg' || 'image/png')) {
      throw new BadRequestException([
        { message: 'This type are not according' },
      ]);
    }
    ...
    if (infoImage.width !== 156 || infoImage.height !== 156) {
      throw new BadRequestException([
        { message: 'Width and height are not according' },
      ]);
    }
    ...
    try {
      ...
      await this.blogsRepository.updateMainForBlogs(
        command.blogId,
        url,
        infoImage,
      );
