...
export class UploadWallpaperForBlogUseCase
  implements ICommandHandler<UploadImageForBlogWallpaperCommand, any>
{
  constructor(
  ...
  async execute(
    command: UploadImageForBlogWallpaperCommand,
    ...
    if (infoImage.width !== 1028 || infoImage.height !== 312) {
      throw new BadRequestException([
        { message: 'Width and heigth are not according' },
      ]);
    }
    ...
    try {
      ...
      await this.blogsRepository.createWallpaperForBlogs(
        command.blogId,
        url,
        infoImage,
      );
