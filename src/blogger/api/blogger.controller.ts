...
import {
  BodyBlogsModel,
  inputModelBlogIdClass,
  inputModelClass,
  inputModelUpdataPost,
} from '../../blogsForSA/dto/blogs.class-pipe';
...
export class BloggerController {
  ...
  async uploadBackgroundWallpaper(
    ...
    const findBlogById: Blogs = await this.blogsQueryRepository.findBlog(
      blogId,
    );
    if (userId !== findBlogById.userId)
      throw new ForbiddenException([
        { message: 'This user does not delong current user' },
      ]);
    ...
    const saveAvatarCommand = new UploadImageForBlogWallpaperCommand(
      userId,
      blogId,
      file.mimetype,
      file.originalname,
    ...
    const result =
      await this.commandBus.execute<UploadImageForBlogWallpaperCommand>(
        saveAvatarCommand,
      );
  ...
  async uploadImageForBlog(
    ...
    const command = new UploadImageForBlogCommand(
      blogId,
      userId,
      file.mimetype,
      file.originalname,
    ...
    const upload = await this.commandBus.execute<UploadImageForBlogCommand>(
      command,
    );
    return upload;
  }
  ...
  async uploadImagesForPost(
    ...
    const findBlogById: Blogs =
      await this.blogsQueryRepository.findBlogByIdAndPostId(blogId);
    const findPostByPostId: Posts | false =
      await this.postsQueryRepository.findPostById(postId);
    if (!findPostByPostId)
      throw new NotFoundException([{ message: 'Post does not found' }]);
    if (userId !== findBlogById.userId)
      throw new ForbiddenException([
        { message: 'This user does not delong current user' },
      ]);
    const command = new UploadImageForPostCommand(
      blogId,
      postId,
      userId,
      file.mimetype,
    ...
    const uploadImageForPost =
      await this.commandBus.execute<UploadImageForPostCommand>(command);
  ...
  async updateBlogsById(
    ...
    const command = new UpdateBlogBloggerForSACommand(
      dto.id,
      inputDateMode,
      user,
    );
    const isUpdateBlog: boolean = await this.commandBus.execute<
      UpdateBlogForSACommand,
      boolean
    >(command);
  ...
  @Delete('blogs/:id')
  @HttpCode(HttpStatus.NO_CONTENT)
  async deleteBlogsById(@Param('id') id: string, @UserDecorator() user: User) {
    const command = new DeleteBlogByIdBloggerForSACommnad(id, user.id);
    const isDeleted: boolean | null = await this.commandBus.execute<
      DeleteBlogByIdBloggerForSACommnad,
      boolean | null
    >(command);
    return isDeleted;
  }
  ...
  async createPostByBlogId(
    ...
    const command = new CreateNewPostForBlogBloggerCommand(
      dto.blogId,
      inputDataModel,
      userId,
    );
    const createNewPost: Posts | null = await this.commandBus.execute<
      CreateNewPostForBlogBloggerCommand,
      Posts | null
    >(command);
    if (!createNewPost)
  ...
  async getPostsByBlogId(
    ...
    const blog = await this.blogsRepositoryForSA.findBlogByIdBlogger(
      dto.blogId,
    );
  ...
  async updatePostByIdWithModel(
    ...
    const command = new UpdateExistingPostByIdWithBlogIdBloggerCommand(
      dto,
      inputModel,
      userId,
    );
    const updateExistingPost: PostsViewModel | null =
      await this.commandBus.execute<
        UpdateExistingPostByIdWithBlogIdBloggerCommand,
        PostsViewModel | null
      >(command);
  ...
  async deletePostByIdWithBlogId(
    ...
    const deletePostById: Promise<any> = await this.commandBus.execute<
  ...
  async createBlog(
    ...
    const createBlog: BlogsViewType | null = await this.commandBus.execute<
      CreateNewBlogForSACommand,
      BlogsViewType | null
    >(command);
    return createBlog;
  }
  ...
  async getBannedUserByBlog(
    ...
    const getBannedUser = await this.commandBus.execute<
      FindBannedUserSpecifyBloggerCommand,
      PaginationType<UserBanBloggerViewType | null>
    >(command);
